...
if [ "$SITE_URL" != "https://justachat.net" ]; then
  ...
  sudo sed -i 's|^GOTRUE_SITE_URL=.*|GOTRUE_SITE_URL=https://justachat.net|' .env
...
if [ "$GOOGLE_ENABLED" != "true" ]; then
  ...
  if grep -q "GOTRUE_EXTERNAL_GOOGLE_ENABLED" .env; then
    sudo sed -i 's|^GOTRUE_EXTERNAL_GOOGLE_ENABLED=.*|GOTRUE_EXTERNAL_GOOGLE_ENABLED=true|' .env
  else
    echo "GOTRUE_EXTERNAL_GOOGLE_ENABLED=true" | sudo tee -a .env > /dev/null
  fi
...
if ! grep -q "GOTRUE_URI_ALLOW_LIST" .env; then
  echo "GOTRUE_URI_ALLOW_LIST=https://justachat.net/**" | sudo tee -a .env > /dev/null
  echo "✓ Added URI allow list"
elif ! grep "GOTRUE_URI_ALLOW_LIST" .env | grep -q "justachat.net"; then
  sudo sed -i 's|^GOTRUE_URI_ALLOW_LIST=.*|GOTRUE_URI_ALLOW_LIST=https://justachat.net/**|' .env
...
if [ -z "$USER_ID" ]; then
  ...
  USER_ID=$(sudo docker exec supabase-db psql -U postgres -t -c \
...
# Verify owner
if [ -n "$USER_ID" ]; then
  ROLE_CHECK=$(sudo docker exec supabase-db psql -U postgres -t -c \
