...
# Usage: sudo bash /var/www/justachat/public/vps-deploy/master-update.sh
# ============================================
...
VPS_ANON_KEY=""
if sudo test -f "$DOCKER_DIR/.env"; then
  VPS_ANON_KEY=$(sudo grep -E '^ANON_KEY=' "$DOCKER_DIR/.env" | head -1 | sed 's/^ANON_KEY=//')
fi
if [ -z "$VPS_ANON_KEY" ]; then
  log_error "Could not read ANON_KEY from $DOCKER_DIR/.env - .env will be incomplete!"
...
UNHEALTHY=$(sudo docker ps --filter "health=unhealthy" --format "{{.Names}}" 2>/dev/null || true)
if [ -n "$UNHEALTHY" ]; then
...
ANON_KEY=""
if sudo test -f "$DOCKER_DIR/.env"; then
  ANON_KEY=$(sudo grep -E '^ANON_KEY=' "$DOCKER_DIR/.env" 2>/dev/null | head -1 | sed 's/^ANON_KEY=//')
  if [ -z "$ANON_KEY" ]; then
    ANON_KEY=$(sudo grep -E '^SUPABASE_ANON_KEY=' "$DOCKER_DIR/.env" 2>/dev/null | head -1 | sed 's/^SUPABASE_ANON_KEY=//')
  fi
fi
...
if [ -z "$ANON_KEY" ]; then
  ...
  sudo grep -i "anon\|key" "$DOCKER_DIR/.env" 2>/dev/null | sed 's/=.*/=[HIDDEN]/' || echo "  File not found"
else
...
FUNC_CONTAINER=$(sudo docker ps --format '{{.Names}}' 2>/dev/null | grep -i "function" | head -1 || echo "")
if [ -n "$FUNC_CONTAINER" ]; then
  echo "Last 10 error lines from $FUNC_CONTAINER:"
  sudo docker logs "$FUNC_CONTAINER" 2>&1 | grep -i "error\|exception\|failed" | tail -10 || echo "  No recent errors found"
else
  log_warn "No functions container found running"
  echo "  Running containers:"
  sudo docker ps --format '  {{.Names}} ({{.Status}})' 2>/dev/null | head -10
fi
