...
if sudo test -d "$FUNC_DIR/irc-gateway"; then
  echo "  Found irc-gateway at: $FUNC_DIR/irc-gateway"
  sudo mkdir -p "$FUNC_DIR/main/irc-gateway"
  sudo cp -f "$FUNC_DIR/irc-gateway/index.ts" "$FUNC_DIR/main/irc-gateway/index.ts"
  ...
  sudo mkdir -p "$FUNC_DIR/main/irc-gateway"
  sudo cp -f "$APP_DIR/supabase/functions/irc-gateway/index.ts" "$FUNC_DIR/main/irc-gateway/index.ts"
...
if sudo test -f "$GATEWAY_FILE"; then
  sudo sed -i 's|import.*from.*"https://deno.land/std.*/http/server.*";||g' "$GATEWAY_FILE"
...
cd "$DOCKER_DIR"
sudo docker compose stop functions 2>/dev/null || sudo docker-compose stop functions 2>/dev/null
sudo docker compose up -d functions 2>/dev/null || sudo docker-compose up -d functions 2>/dev/null
sleep 3
...
ANON_KEY=$(sudo grep "^ANON_KEY=" "$DOCKER_DIR/.env" 2>/dev/null | cut -d'=' -f2 | tr -d '"')
if [ -z "$ANON_KEY" ]; then
  ANON_KEY=$(sudo grep "^SUPABASE_ANON_KEY=" "$DOCKER_DIR/.env" 2>/dev/null | cut -d'=' -f2 | tr -d '"')
fi
if [ -z "$ANON_KEY" ]; then
...
if ss -tlnp | grep -q ':6667'; then
  ...
  sudo fuser -k 6667/tcp 2>/dev/null || true
  sleep 1
fi
...
if command -v ufw &>/dev/null; then
  sudo ufw allow 6667/tcp comment "JAC IRC" 2>/dev/null || true
  sudo ufw allow 6697/tcp comment "JAC IRC SSL" 2>/dev/null || true
  echo -e "  ${GREEN}✓ Firewall rules added for ports 6667 and 6697${NC}"
else
  sudo iptables -I INPUT -p tcp --dport 6667 -j ACCEPT 2>/dev/null || true
  sudo iptables -I INPUT -p tcp --dport 6697 -j ACCEPT 2>/dev/null || true
...
echo "  sudo bash $APP_DIR/public/vps-deploy/diagnose-irc-gateway.sh"
echo ""
