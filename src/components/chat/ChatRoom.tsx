...
const ChatRoom = ({ initialChannelName }: ChatRoomProps) => {
  ...
  return (
    <div className="flex h-screen bg-background relative overflow-hidden">
      ...
      {/* Main Chat Area */}
      <div className="flex flex-col flex-1 min-w-0 h-full overflow-hidden pt-12 lg:pt-0">
        {/* Mobile Header Bar */}
         <div className="flex items-center gap-2 px-2 py-2 border-b border-border bg-card lg:hidden fixed top-0 left-0 right-0 z-50 shrink-0 shadow-sm">
          <Button
            variant="ghost"
            size="icon"
        ...
        <div className="hidden lg:block">
          <ChatHeader 
            onlineCount={(onlineUserIds.size || 1) + 1 /* +1 for bot moderator */}
            topic={topic}
            channelName={currentChannel?.name || 'general'}
