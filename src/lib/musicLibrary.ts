// Comprehensive music library organized by genre
export interface Song {
  title: string;
  artist: string;
  videoId: string;
  genre: string;
}

export interface Genre {
  name: string;
  icon: string;
  songs: Song[];
}

export const MUSIC_LIBRARY: Genre[] = [
  {
    name: 'Lofi',
    icon: '🎧',
    songs: [
      { title: 'Lofi Hip Hop Radio', artist: 'Lofi Girl', videoId: 'jfKfPfyJRdk', genre: 'Lofi' },
      { title: 'Chillhop Radio', artist: 'Chillhop Music', videoId: '5yx6BWlEVcY', genre: 'Lofi' },
      { title: 'Sleepy Lofi', artist: 'Lofi Girl', videoId: 'rUxyKA_-grg', genre: 'Lofi' },
      { title: 'Coffee Shop Vibes', artist: 'Lofi Fruits', videoId: 'lTRiuFIWV54', genre: 'Lofi' },
      { title: 'Late Night Lofi', artist: 'The Jazz Hop Café', videoId: 'kgx4WGK0oNU', genre: 'Lofi' },
      { title: 'Rainy Day Lofi', artist: 'Chillhop', videoId: 'q76bMs-NwRk', genre: 'Lofi' },
      { title: 'Study Session', artist: 'College Music', videoId: 'TURbeWK2wwg', genre: 'Lofi' },
      { title: 'Midnight Study', artist: 'Lofi Records', videoId: 'lP26UCnoH9s', genre: 'Lofi' },
      { title: 'Chill Beats', artist: 'Chillhop', videoId: '-5KAN9_CzSA', genre: 'Lofi' },
      { title: 'Focus Flow', artist: 'Lofi Girl', videoId: 'DWcJFNfaw9c', genre: 'Lofi' },
      { title: 'Dreamy Vibes', artist: 'Dreamhop Music', videoId: '7NOSDKb0HlU', genre: 'Lofi' },
      { title: 'Autumn Leaves', artist: 'Lofi Geek', videoId: 'xjOmErU0vBM', genre: 'Lofi' },
      { title: 'Morning Coffee', artist: 'Chillhop', videoId: 'IUGzY-ihqWc', genre: 'Lofi' },
      { title: 'Cozy Winter', artist: 'The Bootleg Boy', videoId: 'kA97g4X6RoE', genre: 'Lofi' },
      { title: 'Sunset Boulevard', artist: 'Lofi Fruits', videoId: 'hFvf1DLNXF8', genre: 'Lofi' },
      { title: 'Tokyo Nights', artist: 'Nujabes Tribute', videoId: 'WrO9PTpuSSs', genre: 'Lofi' },
      { title: 'Peaceful Mind', artist: 'Purrple Cat', videoId: 'yIQd2Ya0Ziw', genre: 'Lofi' },
      { title: 'Warm Memories', artist: 'Jinsang', videoId: 'E2w5H9e0mgg', genre: 'Lofi' },
      { title: 'Moonlight', artist: 'SwuM', videoId: 'HtUH9z_Oey8', genre: 'Lofi' },
      { title: 'Summer Haze', artist: 'Idealism', videoId: 'V1bFr2SWP1I', genre: 'Lofi' },
    ]
  },
  {
    name: 'Hip Hop',
    icon: '🎤',
    songs: [
      // 2024-2025 Fresh Hip Hop
      { title: 'Not Like Us', artist: 'Kendrick Lamar', videoId: 'T6eK-2OQtew', genre: 'Hip Hop' },
      { title: 'Luther', artist: 'Kendrick Lamar & SZA', videoId: 'JB-r6ffBW4U', genre: 'Hip Hop' },
      { title: 'Squabble Up', artist: 'Kendrick Lamar', videoId: '2bXGIvkkrtg', genre: 'Hip Hop' },
      { title: 'Like That', artist: 'Future, Metro Boomin & Kendrick Lamar', videoId: 'MOmZimH00oo', genre: 'Hip Hop' },
      { title: 'Euphoria', artist: 'Kendrick Lamar', videoId: 'j1sLZqUFHwo', genre: 'Hip Hop' },
      { title: 'HUMBLE.', artist: 'Kendrick Lamar', videoId: 'tvTRZJ-4EyI', genre: 'Hip Hop' },
      { title: 'Big Dawgs', artist: 'Hanumankind & Kalmi', videoId: '7RVz7m8GjR8', genre: 'Hip Hop' },
      { title: 'Type Sh*t', artist: 'Future & Metro Boomin', videoId: 'p4EBdTF2u4Y', genre: 'Hip Hop' },
      { title: 'Carnival', artist: 'Kanye West & Ty Dolla Sign', videoId: 'hG0I7xJT4sk', genre: 'Hip Hop' },
      { title: 'Houdini', artist: 'Eminem', videoId: 'QU-Z-QJPN5k', genre: 'Hip Hop' },
      { title: 'Tobey', artist: 'Eminem ft. Big Sean & BabyTron', videoId: '0u27HbP7mNE', genre: 'Hip Hop' },
      { title: 'We Still Don\'t Trust You', artist: 'Future & Metro Boomin', videoId: 'lR-NZMjqz4A', genre: 'Hip Hop' },
      { title: 'Sicko Mode', artist: 'Travis Scott', videoId: '6ONRf7h3Mdk', genre: 'Hip Hop' },
      { title: 'FE!N', artist: 'Travis Scott ft. Playboi Carti', videoId: 'uASNpnBb0XI', genre: 'Hip Hop' },
      { title: 'MELTDOWN', artist: 'Travis Scott ft. Drake', videoId: 'wL5bAHlxLhM', genre: 'Hip Hop' },
      { title: 'Gods Plan', artist: 'Drake', videoId: 'xpVfcZ0ZcFM', genre: 'Hip Hop' },
      { title: 'Rich Baby Daddy', artist: 'Drake ft. Sexyy Red', videoId: 'RQiRVfJD1Bw', genre: 'Hip Hop' },
      { title: 'Rockstar', artist: 'Post Malone', videoId: 'UceaB4D0jpo', genre: 'Hip Hop' },
      { title: 'Bodak Yellow', artist: 'Cardi B', videoId: 'PEGccV-NOm8', genre: 'Hip Hop' },
      { title: 'Enough (Miami)', artist: 'Cardi B', videoId: 'KQJK9YXvSZQ', genre: 'Hip Hop' },
      { title: 'DNA.', artist: 'Kendrick Lamar', videoId: 'NLZRYQMLDW4', genre: 'Hip Hop' },
      { title: 'Mask Off', artist: 'Future', videoId: 'xvZqHgFz51I', genre: 'Hip Hop' },
      { title: 'Still D.R.E.', artist: 'Dr. Dre ft. Snoop Dogg', videoId: '_CL6n0FJZpk', genre: 'Hip Hop' },
      { title: 'Hot N*gga', artist: 'Bobby Shmurda', videoId: 'vJwKKKd2ZYE', genre: 'Hip Hop' },
      { title: 'XO Tour Llif3', artist: 'Lil Uzi Vert', videoId: 'WrsFXgQk5UI', genre: 'Hip Hop' },
      { title: 'Bad and Boujee', artist: 'Migos', videoId: 'S-sJp1FfG7Q', genre: 'Hip Hop' },
      { title: 'In Da Club', artist: '50 Cent', videoId: '5qm8PH4xAss', genre: 'Hip Hop' },
      { title: 'Stronger', artist: 'Kanye West', videoId: 'PsO6ZnUZI0g', genre: 'Hip Hop' },
      { title: 'Empire State of Mind', artist: 'Jay-Z ft. Alicia Keys', videoId: '0UjsXo9l6I8', genre: 'Hip Hop' },
      { title: 'Old Town Road', artist: 'Lil Nas X', videoId: 'w2Ov5jzm3j8', genre: 'Hip Hop' },
    ]
  },
  {
    name: 'Drill',
    icon: '🔫',
    songs: [
      // 2024-2025 Fresh Drill
      { title: 'Dior', artist: 'Pop Smoke', videoId: 'oorVWW9ywG0', genre: 'Drill' },
      { title: 'For the Night', artist: 'Pop Smoke ft. Lil Baby', videoId: 'Q9pjm4cNsfc', genre: 'Drill' },
      { title: 'What You Know Bout Love', artist: 'Pop Smoke', videoId: 'OUldvjNVAL8', genre: 'Drill' },
      { title: 'Body', artist: 'Tion Wayne & Russ Millions', videoId: '9_E0YoWP5wo', genre: 'Drill' },
      { title: 'Sprinter', artist: 'Central Cee & Dave', videoId: '3qYQLXhUGqg', genre: 'Drill' },
      { title: 'Band4Band', artist: 'Central Cee ft. Lil Baby', videoId: 'aJ-xS8bkTvg', genre: 'Drill' },
      { title: 'Let Go', artist: 'Central Cee', videoId: 'oB_9lsVPfN8', genre: 'Drill' },
      { title: 'Loading', artist: 'Central Cee', videoId: 'mJt-UfECUdg', genre: 'Drill' },
      { title: 'Doja', artist: 'Central Cee', videoId: 'K0wT7EnEYFE', genre: 'Drill' },
      { title: 'The Box', artist: 'Roddy Ricch', videoId: 'uLHqpjW3aDs', genre: 'Drill' },
      { title: 'AHHH HA', artist: 'Lil Durk', videoId: '_kIUq2x0V5k', genre: 'Drill' },
      { title: 'All My Life', artist: 'Lil Durk ft. J. Cole', videoId: 'Z_1lT-UQTOQ', genre: 'Drill' },
      { title: 'Old Days', artist: 'Lil Durk', videoId: 'abZ15xSBlnM', genre: 'Drill' },
      { title: 'The Voice', artist: 'Lil Durk', videoId: 'W5GEvSTvwxE', genre: 'Drill' },
      { title: 'Backdoor', artist: 'Lil Durk', videoId: 'p8kQ7wEHLSY', genre: 'Drill' },
      { title: 'Lifestyle', artist: 'Ice Spice', videoId: '1YFrT91J6fI', genre: 'Drill' },
      { title: 'Munch (Feelin U)', artist: 'Ice Spice', videoId: 'CGIz7dKPJ28', genre: 'Drill' },
      { title: 'Think U The Sh*t (Fart)', artist: 'Ice Spice', videoId: 'p6kKPu5J0ss', genre: 'Drill' },
      { title: 'In Ha Mood', artist: 'Ice Spice', videoId: 'kcitG6PVJ0Q', genre: 'Drill' },
      { title: 'Gata Only', artist: 'FloyyMenor & Cris MJ', videoId: 'bpqWmYMNNqY', genre: 'Drill' },
      { title: 'Crazy Story', artist: 'King Von', videoId: 'qU9OeIgJsLo', genre: 'Drill' },
      { title: 'Armed & Dangerous', artist: 'King Von', videoId: 'gY5-JGpIbvQ', genre: 'Drill' },
      { title: 'War', artist: 'Pop Smoke', videoId: 'rQJj7I85kv0', genre: 'Drill' },
      { title: 'Mood Swings', artist: 'Pop Smoke', videoId: 'EPC0WHhZzZI', genre: 'Drill' },
      { title: 'Got It On Me', artist: 'Pop Smoke', videoId: '8xdMkGT4j_k', genre: 'Drill' },
    ]
  },
  {
    name: 'Rock',
    icon: '🎸',
    songs: [
      { title: 'Bohemian Rhapsody', artist: 'Queen', videoId: 'fJ9rUzIMcZQ', genre: 'Rock' },
      { title: 'Smells Like Teen Spirit', artist: 'Nirvana', videoId: 'hTWKbfoikeg', genre: 'Rock' },
      { title: 'Back In Black', artist: 'AC/DC', videoId: 'pAgnJDJN4VA', genre: 'Rock' },
      { title: 'Sweet Child O\' Mine', artist: 'Guns N\' Roses', videoId: '1w7OgIMMRc4', genre: 'Rock' },
      { title: 'Stairway to Heaven', artist: 'Led Zeppelin', videoId: 'QkF3oxziUI4', genre: 'Rock' },
      { title: 'Hotel California', artist: 'Eagles', videoId: 'EqPtz5qN7HM', genre: 'Rock' },
      { title: 'November Rain', artist: 'Guns N\' Roses', videoId: '8SbUC-UaAxE', genre: 'Rock' },
      { title: 'Thunderstruck', artist: 'AC/DC', videoId: 'v2AC41dglnM', genre: 'Rock' },
      { title: 'Highway to Hell', artist: 'AC/DC', videoId: 'l482T0yNkeo', genre: 'Rock' },
      { title: 'Paradise City', artist: 'Guns N\' Roses', videoId: 'Rbm6GXllBiw', genre: 'Rock' },
      { title: 'Welcome to the Jungle', artist: 'Guns N\' Roses', videoId: 'o1tj2zJ2Wvg', genre: 'Rock' },
      { title: 'Don\'t Stop Believin\'', artist: 'Journey', videoId: '1k8craCGpgs', genre: 'Rock' },
      { title: 'Livin\' on a Prayer', artist: 'Bon Jovi', videoId: 'lDK9QqIzhwk', genre: 'Rock' },
      { title: 'Enter Sandman', artist: 'Metallica', videoId: 'CD-E-LDc384', genre: 'Rock' },
      { title: 'Nothing Else Matters', artist: 'Metallica', videoId: 'tAGnKpE4NCI', genre: 'Rock' },
      { title: 'We Will Rock You', artist: 'Queen', videoId: '-tJYN-eG1zk', genre: 'Rock' },
      { title: 'We Are the Champions', artist: 'Queen', videoId: '04854XqcfCY', genre: 'Rock' },
      { title: 'Crazy Train', artist: 'Ozzy Osbourne', videoId: 'tMDFv5m18Pw', genre: 'Rock' },
      { title: 'Iron Man', artist: 'Black Sabbath', videoId: '8aQRq9hhDWA', genre: 'Rock' },
      { title: 'Smoke on the Water', artist: 'Deep Purple', videoId: 'zUwEIt9ez7M', genre: 'Rock' },
      { title: 'Born to Run', artist: 'Bruce Springsteen', videoId: 'IxuThNgl3YA', genre: 'Rock' },
      { title: 'Free Bird', artist: 'Lynyrd Skynyrd', videoId: 'D0W1v0kOELA', genre: 'Rock' },
      { title: 'Comfortably Numb', artist: 'Pink Floyd', videoId: '_FrOQC-zEog', genre: 'Rock' },
      { title: 'Another Brick in the Wall', artist: 'Pink Floyd', videoId: 'YR5ApYxkU-U', genre: 'Rock' },
      { title: 'Wish You Were Here', artist: 'Pink Floyd', videoId: 'IXdNnw99-Ic', genre: 'Rock' },
      { title: 'Everlong', artist: 'Foo Fighters', videoId: 'eBG7P-K-r1Y', genre: 'Rock' },
      { title: 'The Pretender', artist: 'Foo Fighters', videoId: 'SBjQ9tuuTJQ', genre: 'Rock' },
      { title: 'Under the Bridge', artist: 'Red Hot Chili Peppers', videoId: 'GLvohMXgcBo', genre: 'Rock' },
      { title: 'Californication', artist: 'Red Hot Chili Peppers', videoId: 'YlUKcNNmywk', genre: 'Rock' },
      { title: 'Basket Case', artist: 'Green Day', videoId: 'NUTGr5t3MoY', genre: 'Rock' },
    ]
  },
  {
    name: 'Metal',
    icon: '🤘',
    songs: [
      { title: 'Master of Puppets', artist: 'Metallica', videoId: 'xnKhsTXoKCI', genre: 'Metal' },
      { title: 'One', artist: 'Metallica', videoId: 'WM8bTdBs-cw', genre: 'Metal' },
      { title: 'Raining Blood', artist: 'Slayer', videoId: 'z8ZqFlw6hYg', genre: 'Metal' },
      { title: 'Holy Wars', artist: 'Megadeth', videoId: '9d4ui9q7eDM', genre: 'Metal' },
      { title: 'Peace Sells', artist: 'Megadeth', videoId: 'T0LfHEwEXXw', genre: 'Metal' },
      { title: 'Ace of Spades', artist: 'Motörhead', videoId: '3mbvWn1EY6g', genre: 'Metal' },
      { title: 'The Trooper', artist: 'Iron Maiden', videoId: '2G5rfPISIwo', genre: 'Metal' },
      { title: 'Run to the Hills', artist: 'Iron Maiden', videoId: '86URGgqONvA', genre: 'Metal' },
      { title: 'Painkiller', artist: 'Judas Priest', videoId: 'nM__lPTWThU', genre: 'Metal' },
      { title: 'Breaking the Law', artist: 'Judas Priest', videoId: 'L397TWLwrUU', genre: 'Metal' },
      { title: 'Walk', artist: 'Pantera', videoId: 'AkFqg5wAuFk', genre: 'Metal' },
      { title: 'Cowboys from Hell', artist: 'Pantera', videoId: 'i97OkCXwotE', genre: 'Metal' },
      { title: 'Chop Suey!', artist: 'System of a Down', videoId: 'CSvFpBOe8eY', genre: 'Metal' },
      { title: 'Toxicity', artist: 'System of a Down', videoId: 'iywaBOMvYLI', genre: 'Metal' },
      { title: 'B.Y.O.B.', artist: 'System of a Down', videoId: 'zUzd9KyIDrM', genre: 'Metal' },
      { title: 'Down with the Sickness', artist: 'Disturbed', videoId: '09LTT0xwdfw', genre: 'Metal' },
      { title: 'The Sound of Silence', artist: 'Disturbed', videoId: 'u9Dg-g7t2l4', genre: 'Metal' },
      { title: 'Bodies', artist: 'Drowning Pool', videoId: '04F4xlWSFh0', genre: 'Metal' },
      { title: 'Before I Forget', artist: 'Slipknot', videoId: 'qw2LU1yS7aw', genre: 'Metal' },
      { title: 'Duality', artist: 'Slipknot', videoId: '6fVE8kSM43I', genre: 'Metal' },
    ]
  },
  {
    name: 'EDM',
    icon: '🎛️',
    songs: [
      // 2024-2025 Fresh EDM
      { title: 'Miracle', artist: 'Calvin Harris & Ellie Goulding', videoId: 'G1fqUhW-bik', genre: 'EDM' },
      { title: 'Baby Don\'t Hurt Me', artist: 'David Guetta, Anne-Marie & Coi Leray', videoId: 'dLQI95C26Y8', genre: 'EDM' },
      { title: 'Big', artist: 'David Guetta & Afrojack', videoId: '5JxCxvSjnGs', genre: 'EDM' },
      { title: 'Forever Young', artist: 'David Guetta ft. Ava Max', videoId: 'iqMBFmS2cbo', genre: 'EDM' },
      { title: 'Levels', artist: 'Avicii', videoId: '_ovdm2yX4MA', genre: 'EDM' },
      { title: 'Wake Me Up', artist: 'Avicii', videoId: 'IcrbM1l_BoI', genre: 'EDM' },
      { title: 'Titanium', artist: 'David Guetta ft. Sia', videoId: 'JRfuAukYTKg', genre: 'EDM' },
      { title: 'Animals', artist: 'Martin Garrix', videoId: 'gCYcHz2k5x0', genre: 'EDM' },
      { title: 'Area21', artist: 'Martin Garrix & Maejor', videoId: 'u-dAkH9ZQnY', genre: 'EDM' },
      { title: 'Don\'t You Worry Child', artist: 'Swedish House Mafia', videoId: '1y6smkh6c-0', genre: 'EDM' },
      { title: 'It Gets Better', artist: 'Swedish House Mafia', videoId: 'GcNuuLkWWZ8', genre: 'EDM' },
      { title: 'Clarity', artist: 'Zedd ft. Foxes', videoId: 'IxxstCcJlsc', genre: 'EDM' },
      { title: 'Telos', artist: 'Zedd', videoId: 'r8aaAYJKxOA', genre: 'EDM' },
      { title: 'Lean On', artist: 'Major Lazer & DJ Snake', videoId: 'YqeW9_5kURI', genre: 'EDM' },
      { title: 'Turn Down for What', artist: 'DJ Snake & Lil Jon', videoId: 'HMUDVMiITOU', genre: 'EDM' },
      { title: 'Get Lucky', artist: 'Daft Punk', videoId: '5NV6Rdv1a3I', genre: 'EDM' },
      { title: 'One More Time', artist: 'Daft Punk', videoId: 'FGBhQbmPwH8', genre: 'EDM' },
      { title: 'Faded', artist: 'Alan Walker', videoId: '60ItHLz5WEA', genre: 'EDM' },
      { title: 'Alone', artist: 'Alan Walker', videoId: '1-xGerv5FOk', genre: 'EDM' },
      { title: 'Scared to Be Lonely', artist: 'Martin Garrix & Dua Lipa', videoId: 'e2vBLd5Egnk', genre: 'EDM' },
      { title: 'In The Name Of Love', artist: 'Martin Garrix & Bebe Rexha', videoId: 'RnBT9uUYb1w', genre: 'EDM' },
      { title: 'Something Just Like This', artist: 'Chainsmokers & Coldplay', videoId: 'FM7MFYoylVs', genre: 'EDM' },
      { title: 'Selfcontrol', artist: 'Bebe Rexha & David Guetta', videoId: '18pBQs2e9MA', genre: 'EDM' },
      { title: 'Ferrari', artist: 'James Hype & Miggy Dela Rosa', videoId: 'R-v0Ts8F9Mo', genre: 'EDM' },
      { title: 'Where Have You Been', artist: 'Rihanna', videoId: 'HBxt_v0WF6Y', genre: 'EDM' },
    ]
  },
  {
    name: 'House',
    icon: '🏠',
    songs: [
      { title: 'Finally', artist: 'CeCe Peniston', videoId: 'xk8mm1Qmt-Y', genre: 'House' },
      { title: 'Show Me Love', artist: 'Robin S', videoId: 'Ps2Jc28tQrw', genre: 'House' },
      { title: 'Cola', artist: 'CamelPhat & Elderbrook', videoId: 'zRkWguLk_Gg', genre: 'House' },
      { title: 'Opus', artist: 'Eric Prydz', videoId: 'iRA82xLsb_w', genre: 'House' },
      { title: 'One Kiss', artist: 'Calvin Harris & Dua Lipa', videoId: 'DkeiKbqa02g', genre: 'House' },
      { title: 'Summer', artist: 'Calvin Harris', videoId: 'ebXbLfLACGM', genre: 'House' },
      { title: 'This Is What You Came For', artist: 'Calvin Harris ft. Rihanna', videoId: 'kOkQ4T5WO9E', genre: 'House' },
      { title: 'How Deep Is Your Love', artist: 'Calvin Harris & Disciples', videoId: 'EgqUJOudrcM', genre: 'House' },
      { title: 'Promises', artist: 'Calvin Harris & Sam Smith', videoId: 'b3rkHsAoeu4', genre: 'House' },
      { title: 'Deep Down', artist: 'Alok', videoId: 'bkk-EpstAjE', genre: 'House' },
      { title: 'Hear Me Now', artist: 'Alok & Bruno Martini', videoId: 'fkDpVgGA8cQ', genre: 'House' },
      { title: 'Runaway (U & I)', artist: 'Galantis', videoId: '5XR7naZ_zZA', genre: 'House' },
      { title: 'No Money', artist: 'Galantis', videoId: 'xMWtLaWgFqQ', genre: 'House' },
      { title: 'Pjanoo', artist: 'Eric Prydz', videoId: 'llfNRGQ7Tlg', genre: 'House' },
      { title: 'I Gotta Feeling', artist: 'Black Eyed Peas', videoId: 'uSD4vsh1zDA', genre: 'House' },
      { title: 'Insomnia', artist: 'Faithless', videoId: 'P8JEm4d6Wu4', genre: 'House' },
      { title: 'Finally Moving', artist: 'Pretty Lights', videoId: 'Sk9XYQMRiLY', genre: 'House' },
      { title: 'Be Right There', artist: 'Diplo & Sleepy Tom', videoId: 'tg00YEETFzg', genre: 'House' },
      { title: 'Rather Be', artist: 'Clean Bandit', videoId: 'm-M1AtrxztU', genre: 'House' },
      { title: 'Rockabye', artist: 'Clean Bandit ft. Anne-Marie', videoId: 'papuvlVeZg8', genre: 'House' },
    ]
  },
  {
    name: 'Dubstep',
    icon: '💥',
    songs: [
      { title: 'Scary Monsters and Nice Sprites', artist: 'Skrillex', videoId: 'WSeNSzJ2-Jw', genre: 'Dubstep' },
      { title: 'Bangarang', artist: 'Skrillex', videoId: 'YJVmu6yttiw', genre: 'Dubstep' },
      { title: 'First of the Year', artist: 'Skrillex', videoId: '2cXDgFwE13g', genre: 'Dubstep' },
      { title: 'Cinema', artist: 'Skrillex Remix', videoId: 'Ua0KpfJsxKo', genre: 'Dubstep' },
      { title: 'Bass Cannon', artist: 'Flux Pavilion', videoId: 'gYGioWinQQE', genre: 'Dubstep' },
      { title: 'I Can\'t Stop', artist: 'Flux Pavilion', videoId: 'hzExWz7KP5M', genre: 'Dubstep' },
      { title: 'Cracks', artist: 'Flux Pavilion', videoId: 'K1VLaXoRRdk', genre: 'Dubstep' },
      { title: 'Bonfire', artist: 'Knife Party', videoId: 'e-IWRmpefzE', genre: 'Dubstep' },
      { title: 'Internet Friends', artist: 'Knife Party', videoId: 'gcejLp72iCE', genre: 'Dubstep' },
      { title: 'Promises', artist: 'Nero', videoId: 'llDikI2hTtk', genre: 'Dubstep' },
      { title: 'Must Be the Feeling', artist: 'Nero', videoId: 'PYGS0yyPl5E', genre: 'Dubstep' },
      { title: 'Red Lips', artist: 'Skrillex Remix', videoId: 'Cy4CKKP9xHw', genre: 'Dubstep' },
      { title: 'Still Getting It', artist: 'Foreign Beggars ft. Skrillex', videoId: 'bw2o_Go4QWI', genre: 'Dubstep' },
      { title: 'Bassline Kickin', artist: 'Zomboy', videoId: 'aUv6nHrjxM8', genre: 'Dubstep' },
      { title: 'Like A Bitch', artist: 'Zomboy', videoId: 'RIjHlJfXzSk', genre: 'Dubstep' },
      { title: 'Louder', artist: 'Doctor P & Flux Pavilion', videoId: 'rMR8Oo_pvqM', genre: 'Dubstep' },
      { title: 'Tetris', artist: 'Doctor P', videoId: 'OBq8bqPwV68', genre: 'Dubstep' },
      { title: 'Core', artist: 'RL Grime', videoId: 'sbawxvG3lo0', genre: 'Dubstep' },
      { title: 'Tell Me', artist: 'RL Grime', videoId: 'E_Qj45GfsFg', genre: 'Dubstep' },
      { title: 'Midnight Tyrannosaurus Mix', artist: 'Midnight T', videoId: '0VG1aP3ym30', genre: 'Dubstep' },
    ]
  },
  {
    name: 'Jazz',
    icon: '🎷',
    songs: [
      { title: 'Take Five', artist: 'Dave Brubeck', videoId: 'vmDDOFXSgAs', genre: 'Jazz' },
      { title: 'So What', artist: 'Miles Davis', videoId: 'ylXk1LBvIqU', genre: 'Jazz' },
      { title: 'Blue in Green', artist: 'Miles Davis', videoId: 'PoPL7BExSQU', genre: 'Jazz' },
      { title: 'My Favorite Things', artist: 'John Coltrane', videoId: 'qWG2dsXV5HI', genre: 'Jazz' },
      { title: 'A Love Supreme', artist: 'John Coltrane', videoId: 'll3CMgiUPuU', genre: 'Jazz' },
      { title: 'All Blues', artist: 'Miles Davis', videoId: '-488UORrfJ0', genre: 'Jazz' },
      { title: 'Fly Me to the Moon', artist: 'Frank Sinatra', videoId: 'ZEcqHA7dbwM', genre: 'Jazz' },
      { title: 'What a Wonderful World', artist: 'Louis Armstrong', videoId: 'VqhCQZaH4Vs', genre: 'Jazz' },
      { title: 'Autumn Leaves', artist: 'Nat King Cole', videoId: 'PxNfY1GwEP4', genre: 'Jazz' },
      { title: 'Round Midnight', artist: 'Thelonious Monk', videoId: 'ojtuDUbRoxI', genre: 'Jazz' },
      { title: 'Summertime', artist: 'Ella Fitzgerald', videoId: 'ysov6iV2vLs', genre: 'Jazz' },
      { title: 'Feeling Good', artist: 'Nina Simone', videoId: 'D5Y11hwjMNs', genre: 'Jazz' },
      { title: 'Misty', artist: 'Erroll Garner', videoId: '9UmLsu0Z3L8', genre: 'Jazz' },
      { title: 'In a Sentimental Mood', artist: 'Duke Ellington', videoId: 'klCj8YybS5A', genre: 'Jazz' },
      { title: 'Strange Fruit', artist: 'Billie Holiday', videoId: 'Web007rzSOI', genre: 'Jazz' },
      { title: 'Watermelon Man', artist: 'Herbie Hancock', videoId: '4bjPlBC4h_8', genre: 'Jazz' },
      { title: 'Cantaloupe Island', artist: 'Herbie Hancock', videoId: '8B1oIXGX0Io', genre: 'Jazz' },
      { title: 'Girl from Ipanema', artist: 'Stan Getz', videoId: 'j8VPmtyLqSY', genre: 'Jazz' },
      { title: 'Cafe Music Radio', artist: 'Cafe Music BGM', videoId: 'Dx5qFachd3A', genre: 'Jazz' },
      { title: 'Smooth Jazz Radio', artist: 'Smooth Jazz', videoId: 'M0gvVZs0wTA', genre: 'Jazz' },
    ]
  },
  {
    name: 'Pop',
    icon: '🎵',
    songs: [
      // 2024-2025 Fresh Pop
      { title: 'Espresso', artist: 'Sabrina Carpenter', videoId: 'eVli-tstM5E', genre: 'Pop' },
      { title: 'Please Please Please', artist: 'Sabrina Carpenter', videoId: 'cF1Na4AIecM', genre: 'Pop' },
      { title: 'Taste', artist: 'Sabrina Carpenter', videoId: '1AiGsVuxKc8', genre: 'Pop' },
      { title: 'Die With A Smile', artist: 'Lady Gaga & Bruno Mars', videoId: 'kPa7bsKwL-c', genre: 'Pop' },
      { title: 'APT.', artist: 'ROSE & Bruno Mars', videoId: 'ekr2nIex040', genre: 'Pop' },
      { title: 'Birds of a Feather', artist: 'Billie Eilish', videoId: 'V55XSKkNx7k', genre: 'Pop' },
      { title: 'LUNCH', artist: 'Billie Eilish', videoId: 'MoXvm4KbWUw', genre: 'Pop' },
      { title: 'Blinding Lights', artist: 'The Weeknd', videoId: '4NRXx6U8ABQ', genre: 'Pop' },
      { title: 'Dancing In The Flames', artist: 'The Weeknd', videoId: 'gHQtw_5ZBMU', genre: 'Pop' },
      { title: 'Shape of You', artist: 'Ed Sheeran', videoId: 'JGwWNGJdvx8', genre: 'Pop' },
      { title: 'Beautiful Things', artist: 'Benson Boone', videoId: 'Eo5antGaXHg', genre: 'Pop' },
      { title: 'Lose Control', artist: 'Teddy Swims', videoId: 'nM1kxu4eJgg', genre: 'Pop' },
      { title: 'Good Luck Babe!', artist: 'Chappell Roan', videoId: 'B1-fJJNBkNQ', genre: 'Pop' },
      { title: 'HOT TO GO!', artist: 'Chappell Roan', videoId: 'xLPJM1S7xmg', genre: 'Pop' },
      { title: 'Pink Pony Club', artist: 'Chappell Roan', videoId: 'mECkd64mQNI', genre: 'Pop' },
      { title: 'Too Sweet', artist: 'Hozier', videoId: 'fevKSdtbL-s', genre: 'Pop' },
      { title: 'Fortnight', artist: 'Taylor Swift ft. Post Malone', videoId: 'q3zqJs7JUCQ', genre: 'Pop' },
      { title: 'Cruel Summer', artist: 'Taylor Swift', videoId: 'ic8j13piAhQ', genre: 'Pop' },
      { title: 'Anti-Hero', artist: 'Taylor Swift', videoId: 'b1kbLwvqugk', genre: 'Pop' },
      { title: 'Uptown Funk', artist: 'Bruno Mars', videoId: 'OPf0YbXqDm0', genre: 'Pop' },
      { title: 'Levitating', artist: 'Dua Lipa', videoId: 'TUVcZfQe-Kw', genre: 'Pop' },
      { title: 'Don\'t Start Now', artist: 'Dua Lipa', videoId: 'oygrmJFKYZY', genre: 'Pop' },
      { title: 'Houdini', artist: 'Dua Lipa', videoId: 'suAR1PYFNYA', genre: 'Pop' },
      { title: 'As It Was', artist: 'Harry Styles', videoId: 'H5v3kku4y6Q', genre: 'Pop' },
      { title: 'Flowers', artist: 'Miley Cyrus', videoId: 'G7KNmW9a75Y', genre: 'Pop' },
      { title: 'Stick Season', artist: 'Noah Kahan', videoId: 'oeQeFRaEMAQ', genre: 'Pop' },
      { title: 'Greedy', artist: 'Tate McRae', videoId: '8HnLRrQ3RS4', genre: 'Pop' },
      { title: 'Hello', artist: 'Adele', videoId: 'YQHsXMglC9A', genre: 'Pop' },
      { title: 'Stay', artist: 'The Kid LAROI & Justin Bieber', videoId: 'kTJczUoc26U', genre: 'Pop' },
      { title: 'Peaches', artist: 'Justin Bieber', videoId: 'tQ0yjYUFKAE', genre: 'Pop' },
    ]
  },
  {
    name: 'R&B',
    icon: '💜',
    songs: [
      { title: 'Blinding Lights', artist: 'The Weeknd', videoId: '4NRXx6U8ABQ', genre: 'R&B' },
      { title: 'Save Your Tears', artist: 'The Weeknd', videoId: 'XXYlFuWEuKI', genre: 'R&B' },
      { title: 'Starboy', artist: 'The Weeknd', videoId: '34Na4j8AVgA', genre: 'R&B' },
      { title: 'The Hills', artist: 'The Weeknd', videoId: 'yzTuBuRdAyA', genre: 'R&B' },
      { title: 'Earned It', artist: 'The Weeknd', videoId: 'waU75jdUnYw', genre: 'R&B' },
      { title: 'Redbone', artist: 'Childish Gambino', videoId: 'Kp7eSUzdc0E', genre: 'R&B' },
      { title: 'Location', artist: 'Khalid', videoId: 'by3yRdlQvzs', genre: 'R&B' },
      { title: 'Young Dumb & Broke', artist: 'Khalid', videoId: 'IPfJnp1guPc', genre: 'R&B' },
      { title: 'Talk', artist: 'Khalid', videoId: 'hE2Ira-Cwxo', genre: 'R&B' },
      { title: 'Best Part', artist: 'Daniel Caesar ft. H.E.R.', videoId: 'vBy7FaapGRo', genre: 'R&B' },
      { title: 'Get You', artist: 'Daniel Caesar ft. Kali Uchis', videoId: 'uQFVqltOXRg', genre: 'R&B' },
      { title: 'Focus', artist: 'H.E.R.', videoId: 'l5qb5eL8uJg', genre: 'R&B' },
      { title: 'Drunk in Love', artist: 'Beyoncé', videoId: 'p1JPKLa-Ofc', genre: 'R&B' },
      { title: 'Crazy in Love', artist: 'Beyoncé', videoId: 'ViwtNLUqkMY', genre: 'R&B' },
      { title: 'Formation', artist: 'Beyoncé', videoId: 'WDZJPJV__bQ', genre: 'R&B' },
      { title: 'No Guidance', artist: 'Chris Brown ft. Drake', videoId: 'AoE0rlEf-iQ', genre: 'R&B' },
      { title: 'Go Crazy', artist: 'Chris Brown & Young Thug', videoId: 'VTn9QLnftnY', genre: 'R&B' },
      { title: 'Needed Me', artist: 'Rihanna', videoId: 'cMPoHEf9K4g', genre: 'R&B' },
      { title: 'Work', artist: 'Rihanna ft. Drake', videoId: 'HL1UzIK-flA', genre: 'R&B' },
      { title: 'Kiss It Better', artist: 'Rihanna', videoId: '49lY0HqqUVc', genre: 'R&B' },
    ]
  },
  {
    name: 'Synthwave',
    icon: '🌆',
    songs: [
      { title: 'Nightcall', artist: 'Kavinsky', videoId: 'MV_3Dpw-BRY', genre: 'Synthwave' },
      { title: 'A Real Hero', artist: 'College & Electric Youth', videoId: '-DSVDcw6iW8', genre: 'Synthwave' },
      { title: 'Tech Noir', artist: 'Gunship', videoId: 'c80dWbiOR0Y', genre: 'Synthwave' },
      { title: 'Dark All Day', artist: 'Gunship', videoId: '60ruvzfXQoE', genre: 'Synthwave' },
      { title: 'Resonance', artist: 'Home', videoId: '8GW6sLrK40k', genre: 'Synthwave' },
      { title: 'Turbo Killer', artist: 'Carpenter Brut', videoId: 'er416Ad3R1g', genre: 'Synthwave' },
      { title: 'Le Perv', artist: 'Carpenter Brut', videoId: 'RYtVf0wvPpc', genre: 'Synthwave' },
      { title: 'Roller Mobster', artist: 'Carpenter Brut', videoId: 'qFfybn_W8Ak', genre: 'Synthwave' },
      { title: 'Blood Red', artist: 'Waterfront Dining', videoId: 'PuQe8_L0E2E', genre: 'Synthwave' },
      { title: 'Blizzard', artist: 'Kavinsky', videoId: 'a2CUUloT2YA', genre: 'Synthwave' },
      { title: 'Odd Look', artist: 'Kavinsky', videoId: 'qT0iQRjHbW4', genre: 'Synthwave' },
      { title: 'Crystals', artist: 'M.O.O.N.', videoId: '-8X0sJJRKq0', genre: 'Synthwave' },
      { title: 'Hydrogen', artist: 'M.O.O.N.', videoId: 'oKD-MVfC9Ag', genre: 'Synthwave' },
      { title: 'The Encounter', artist: 'Danger', videoId: '0J5PsKR5QHM', genre: 'Synthwave' },
      { title: 'Sunset', artist: 'The Midnight', videoId: 'w2D4iCzRW7E', genre: 'Synthwave' },
      { title: 'Los Angeles', artist: 'The Midnight', videoId: 'E71DLWB50jU', genre: 'Synthwave' },
      { title: 'Daytona', artist: 'Lazerhawk', videoId: 'Ewe5rKP2EpQ', genre: 'Synthwave' },
      { title: 'Overdrive', artist: 'Lazerhawk', videoId: 'Pkj9yxd6XCA', genre: 'Synthwave' },
      { title: 'Synthwave Radio', artist: 'Synthwave Goose', videoId: '4xDzrJKXOOY', genre: 'Synthwave' },
      { title: 'Electric Dreams', artist: 'Synthwave Mix', videoId: 'rDBbaGCCIhk', genre: 'Synthwave' },
    ]
  },
  {
    name: 'Classical',
    icon: '🎻',
    songs: [
      { title: 'Für Elise', artist: 'Beethoven', videoId: '_mVW8tgGY_w', genre: 'Classical' },
      { title: 'Moonlight Sonata', artist: 'Beethoven', videoId: '4Tr0otuiQuU', genre: 'Classical' },
      { title: 'Symphony No. 5', artist: 'Beethoven', videoId: 'fOk8Tm815lE', genre: 'Classical' },
      { title: 'Canon in D', artist: 'Pachelbel', videoId: 'NlprozGcs80', genre: 'Classical' },
      { title: 'Four Seasons - Spring', artist: 'Vivaldi', videoId: 'l-dYNttdgl0', genre: 'Classical' },
      { title: 'Four Seasons - Winter', artist: 'Vivaldi', videoId: 'TZCfydWF48c', genre: 'Classical' },
      { title: 'Clair de Lune', artist: 'Debussy', videoId: 'CvFH_6DNRCY', genre: 'Classical' },
      { title: 'Nocturne Op. 9 No. 2', artist: 'Chopin', videoId: '9E6b3swbnWg', genre: 'Classical' },
      { title: 'Waltz in C# Minor', artist: 'Chopin', videoId: 'hOcryGEw1NY', genre: 'Classical' },
      { title: 'Swan Lake', artist: 'Tchaikovsky', videoId: '9cNQFB0TDfY', genre: 'Classical' },
      { title: 'The Nutcracker Suite', artist: 'Tchaikovsky', videoId: 'M8J8urC_8Jw', genre: 'Classical' },
      { title: 'Eine Kleine Nachtmusik', artist: 'Mozart', videoId: 'oy2zDJPIgwc', genre: 'Classical' },
      { title: 'Requiem - Lacrimosa', artist: 'Mozart', videoId: 'k1-TrAvp_xs', genre: 'Classical' },
      { title: 'The Blue Danube', artist: 'Strauss', videoId: '_CTYymbbEL4', genre: 'Classical' },
      { title: 'In the Hall of the Mountain King', artist: 'Grieg', videoId: 'kLp_Hh6DKWc', genre: 'Classical' },
      { title: 'Ride of the Valkyries', artist: 'Wagner', videoId: 'GGU1P6lBW6Q', genre: 'Classical' },
      { title: 'Habanera - Carmen', artist: 'Bizet', videoId: 'K2snTkaD64U', genre: 'Classical' },
      { title: 'Air on the G String', artist: 'Bach', videoId: 'pzlw6fUux4o', genre: 'Classical' },
      { title: 'Toccata and Fugue', artist: 'Bach', videoId: 'ho9rZjlsyYY', genre: 'Classical' },
      { title: 'Classical Piano Radio', artist: 'Rousseau', videoId: 'klPZIGQcrHA', genre: 'Classical' },
    ]
  },
  {
    name: 'Trap',
    icon: '🔊',
    songs: [
      // 2024-2025 Fresh Trap
      { title: 'HUMBLE.', artist: 'Kendrick Lamar', videoId: 'tvTRZJ-4EyI', genre: 'Trap' },
      { title: 'Like That', artist: 'Future, Metro Boomin & Kendrick', videoId: 'MOmZimH00oo', genre: 'Trap' },
      { title: 'Type Sh*t', artist: 'Future, Metro Boomin & Playboi Carti', videoId: 'p4EBdTF2u4Y', genre: 'Trap' },
      { title: 'We Still Don\'t Trust You', artist: 'Future & Metro Boomin ft. The Weeknd', videoId: 'lR-NZMjqz4A', genre: 'Trap' },
      { title: 'Goosebumps', artist: 'Travis Scott', videoId: 'Dst9gZkq1a8', genre: 'Trap' },
      { title: 'Highest in the Room', artist: 'Travis Scott', videoId: 'tfSS1e3kYeo', genre: 'Trap' },
      { title: 'FE!N', artist: 'Travis Scott ft. Playboi Carti', videoId: 'uASNpnBb0XI', genre: 'Trap' },
      { title: 'MY EYES', artist: 'Travis Scott ft. Sampha', videoId: 'I6TyS5FYz8c', genre: 'Trap' },
      { title: 'TELEKINESIS', artist: 'Travis Scott ft. Future', videoId: '9rwHwGWoGgs', genre: 'Trap' },
      { title: 'Bad and Boujee', artist: 'Migos', videoId: 'S-sJp1FfG7Q', genre: 'Trap' },
      { title: 'XO Tour Llif3', artist: 'Lil Uzi Vert', videoId: 'WrsFXgQk5UI', genre: 'Trap' },
      { title: 'Pink Tape', artist: 'Lil Uzi Vert', videoId: 'QhI1UIw0A08', genre: 'Trap' },
      { title: 'Mask Off', artist: 'Future', videoId: 'xvZqHgFz51I', genre: 'Trap' },
      { title: 'rockstar', artist: 'Post Malone', videoId: 'UceaB4D0jpo', genre: 'Trap' },
      { title: 'I Had Some Help', artist: 'Post Malone ft. Morgan Wallen', videoId: 'jkOEu4CdNow', genre: 'Trap' },
      { title: 'Drip Too Hard', artist: 'Lil Baby & Gunna', videoId: 'THChnxOzV3w', genre: 'Trap' },
      { title: 'Crazy', artist: 'Lil Baby', videoId: 'RQfCYxMJkN4', genre: 'Trap' },
      { title: 'Wham', artist: 'Lil Baby', videoId: 'FujVNrJJf4M', genre: 'Trap' },
      { title: 'Real Spill', artist: 'Lil Baby', videoId: 'Ee1w7pMjhBM', genre: 'Trap' },
      { title: 'Carnival', artist: 'Kanye West & Ty Dolla Sign', videoId: 'hG0I7xJT4sk', genre: 'Trap' },
      { title: 'VULTURES', artist: 'Kanye West & Ty Dolla Sign', videoId: 'kTsGHG0VJyE', genre: 'Trap' },
      { title: 'I\'m The One', artist: 'DJ Khaled', videoId: 'weeI1G46q0o', genre: 'Trap' },
      { title: 'Wild Thoughts', artist: 'DJ Khaled', videoId: 'fyMhvkC3A84', genre: 'Trap' },
      { title: 'Congratulations', artist: 'Post Malone', videoId: 'SC4xMk98Pdc', genre: 'Trap' },
      { title: 'White Iverson', artist: 'Post Malone', videoId: 'SLsTskUWUP4', genre: 'Trap' },
    ]
  },
  {
    name: 'Top Hits',
    icon: '🔥',
    songs: [
      // 2025 Latest Chart Toppers
      { title: 'APT.', artist: 'ROSE & Bruno Mars', videoId: 'ekr2nIex040', genre: 'Top Hits' },
      { title: 'Die With A Smile', artist: 'Lady Gaga & Bruno Mars', videoId: 'kPa7bsKwL-c', genre: 'Top Hits' },
      { title: 'Luther', artist: 'Kendrick Lamar & SZA', videoId: 'JB-r6ffBW4U', genre: 'Top Hits' },
      { title: 'Squabble Up', artist: 'Kendrick Lamar', videoId: '2bXGIvkkrtg', genre: 'Top Hits' },
      { title: 'Espresso', artist: 'Sabrina Carpenter', videoId: 'eVli-tstM5E', genre: 'Top Hits' },
      { title: 'Please Please Please', artist: 'Sabrina Carpenter', videoId: 'cF1Na4AIecM', genre: 'Top Hits' },
      { title: 'A Bar Song (Tipsy)', artist: 'Shaboozey', videoId: 'p6FAhnvVSfc', genre: 'Top Hits' },
      { title: 'Not Like Us', artist: 'Kendrick Lamar', videoId: 'T6eK-2OQtew', genre: 'Top Hits' },
      { title: 'Birds of a Feather', artist: 'Billie Eilish', videoId: 'V55XSKkNx7k', genre: 'Top Hits' },
      { title: 'Lose Control', artist: 'Teddy Swims', videoId: 'nM1kxu4eJgg', genre: 'Top Hits' },
      { title: 'Beautiful Things', artist: 'Benson Boone', videoId: 'Eo5antGaXHg', genre: 'Top Hits' },
      { title: 'Million Dollar Baby', artist: 'Tommy Richman', videoId: 'FJ9ELkfqJpc', genre: 'Top Hits' },
      { title: 'I Had Some Help', artist: 'Post Malone ft. Morgan Wallen', videoId: 'jkOEu4CdNow', genre: 'Top Hits' },
      { title: 'Taste', artist: 'Sabrina Carpenter', videoId: '1AiGsVuxKc8', genre: 'Top Hits' },
      { title: 'Too Sweet', artist: 'Hozier', videoId: 'fevKSdtbL-s', genre: 'Top Hits' },
      { title: 'Fortnight', artist: 'Taylor Swift ft. Post Malone', videoId: 'q3zqJs7JUCQ', genre: 'Top Hits' },
      { title: 'LUNCH', artist: 'Billie Eilish', videoId: 'MoXvm4KbWUw', genre: 'Top Hits' },
      { title: 'Good Luck Babe!', artist: 'Chappell Roan', videoId: 'B1-fJJNBkNQ', genre: 'Top Hits' },
      { title: 'HOT TO GO!', artist: 'Chappell Roan', videoId: 'xLPJM1S7xmg', genre: 'Top Hits' },
      { title: 'Feather', artist: 'Sabrina Carpenter', videoId: 'oPiITWkUSKg', genre: 'Top Hits' },
      { title: 'Stick Season', artist: 'Noah Kahan', videoId: 'oeQeFRaEMAQ', genre: 'Top Hits' },
      { title: 'Greedy', artist: 'Tate McRae', videoId: '8HnLRrQ3RS4', genre: 'Top Hits' },
      { title: 'Paint The Town Red', artist: 'Doja Cat', videoId: 'sNbvmS0hRSc', genre: 'Top Hits' },
      { title: 'Cruel Summer', artist: 'Taylor Swift', videoId: 'ic8j13piAhQ', genre: 'Top Hits' },
      { title: 'Flowers', artist: 'Miley Cyrus', videoId: 'G7KNmW9a75Y', genre: 'Top Hits' },
      { title: 'Unholy', artist: 'Sam Smith & Kim Petras', videoId: 'Uq9gPaIzbe8', genre: 'Top Hits' },
      { title: 'About Damn Time', artist: 'Lizzo', videoId: 'IXXxciRUMzE', genre: 'Top Hits' },
      { title: 'Heat Waves', artist: 'Glass Animals', videoId: 'mRD0-GxqHVo', genre: 'Top Hits' },
      { title: 'Industry Baby', artist: 'Lil Nas X & Jack Harlow', videoId: 'UTHLKHL_whs', genre: 'Top Hits' },
      { title: 'Dancing In The Flames', artist: 'The Weeknd', videoId: 'gHQtw_5ZBMU', genre: 'Top Hits' },
    ]
  },
  {
    name: 'Country Hits',
    icon: '🤠',
    songs: [
      { title: 'Last Night', artist: 'Morgan Wallen', videoId: 'YdsDVRPxoPI', genre: 'Country Hits' },
      { title: 'Fast Car', artist: 'Luke Combs', videoId: 'oHdKF0fE9W0', genre: 'Country Hits' },
      { title: 'Thinkin Bout Me', artist: 'Morgan Wallen', videoId: 'RQHcxT5Qbuk', genre: 'Country Hits' },
      { title: 'Try That In A Small Town', artist: 'Jason Aldean', videoId: 'LWvvawLDVlQ', genre: 'Country Hits' },
      { title: 'Man Made A Bar', artist: 'Morgan Wallen ft. Eric Church', videoId: 'iNy76N6fPxE', genre: 'Country Hits' },
      { title: 'Whiskey Glasses', artist: 'Morgan Wallen', videoId: 'HUWGNqIJOYo', genre: 'Country Hits' },
      { title: 'Wasted On You', artist: 'Morgan Wallen', videoId: 'oqIFFNq5jWM', genre: 'Country Hits' },
      { title: 'Heartfirst', artist: 'Kelsea Ballerini', videoId: 'pI0uZLCXxGE', genre: 'Country Hits' },
      { title: 'Need A Favor', artist: 'Jelly Roll', videoId: '1dDzdqMXD8o', genre: 'Country Hits' },
      { title: 'Save Me', artist: 'Jelly Roll', videoId: 'FxFNprPOdss', genre: 'Country Hits' },
      { title: 'Son Of A Sinner', artist: 'Jelly Roll', videoId: 'hJRZoJn8e1o', genre: 'Country Hits' },
      { title: 'Something in the Orange', artist: 'Zach Bryan', videoId: 'YO5Hzejnmpw', genre: 'Country Hits' },
      { title: 'I Remember Everything', artist: 'Zach Bryan ft. Kacey Musgraves', videoId: 'nxVf2pQPiNc', genre: 'Country Hits' },
      { title: 'Burn It Down', artist: 'Parker McCollum', videoId: 'XfN7tQPjFNs', genre: 'Country Hits' },
      { title: 'You Proof', artist: 'Morgan Wallen', videoId: 'eiVjV_P-lME', genre: 'Country Hits' },
      { title: 'Beer Never Broke My Heart', artist: 'Luke Combs', videoId: '2kL3HAbYkac', genre: 'Country Hits' },
      { title: 'Forever After All', artist: 'Luke Combs', videoId: 'jRCPEDOLr6c', genre: 'Country Hits' },
      { title: 'Buy Dirt', artist: 'Jordan Davis ft. Luke Bryan', videoId: 'kP_PVlwkM0s', genre: 'Country Hits' },
      { title: 'Fancy Like', artist: 'Walker Hayes', videoId: 'G_zuB-ogIBw', genre: 'Country Hits' },
      { title: 'Wait In The Truck', artist: 'HARDY ft. Lainey Wilson', videoId: 'DmDcLZL-_1k', genre: 'Country Hits' },
    ]
  },
  {
    name: 'Latin Hits',
    icon: '🌴',
    songs: [
      { title: 'Despacito', artist: 'Luis Fonsi ft. Daddy Yankee', videoId: 'kJQP7kiw5Fk', genre: 'Latin Hits' },
      { title: 'Dákiti', artist: 'Bad Bunny & Jhay Cortez', videoId: 'TmKh7lAwnBI', genre: 'Latin Hits' },
      { title: 'Tití Me Preguntó', artist: 'Bad Bunny', videoId: '8SxGKp-U6Fc', genre: 'Latin Hits' },
      { title: 'Monaco', artist: 'Bad Bunny', videoId: 'Y1Gs97h2rbQ', genre: 'Latin Hits' },
      { title: 'WHERE SHE GOES', artist: 'Bad Bunny', videoId: 'JKYXiNmxjDg', genre: 'Latin Hits' },
      { title: 'Efecto', artist: 'Bad Bunny', videoId: 'XekMiglTEGM', genre: 'Latin Hits' },
      { title: 'Pepas', artist: 'Farruko', videoId: 'Bvi_0c4bDQM', genre: 'Latin Hits' },
      { title: 'Tusa', artist: 'Karol G & Nicki Minaj', videoId: 'tbneQDc0mV0', genre: 'Latin Hits' },
      { title: 'TQG', artist: 'Karol G & Shakira', videoId: '_w8SY_9yO8k', genre: 'Latin Hits' },
      { title: 'Mientras Me Curo del Cora', artist: 'Karol G', videoId: 'QP_VVgF4Skc', genre: 'Latin Hits' },
      { title: 'Mi Ex Tenía Razón', artist: 'Karol G', videoId: 'gTJZmb5Vl8I', genre: 'Latin Hits' },
      { title: 'La Bebe', artist: 'Yng Lvcas & Peso Pluma', videoId: 'NSQZqVsaKWY', genre: 'Latin Hits' },
      { title: 'Ella Baila Sola', artist: 'Eslabon Armado & Peso Pluma', videoId: 'n7aI2dMfAzI', genre: 'Latin Hits' },
      { title: 'Shakira: Bzrp Music Sessions 53', artist: 'Bizarrap & Shakira', videoId: 'CocEMWdc7Ck', genre: 'Latin Hits' },
      { title: 'Te Felicito', artist: 'Shakira & Rauw Alejandro', videoId: 'ALQP3oLiAWI', genre: 'Latin Hits' },
      { title: 'Hawái', artist: 'Maluma', videoId: '8dSJKefCfHo', genre: 'Latin Hits' },
      { title: 'Ojitos Lindos', artist: 'Bad Bunny & Bomba Estéreo', videoId: 'U17fZZPRZaU', genre: 'Latin Hits' },
      { title: 'Callaíta', artist: 'Bad Bunny', videoId: 'FNXJy4AQMPk', genre: 'Latin Hits' },
      { title: 'Me Porto Bonito', artist: 'Bad Bunny ft. Chencho Corleone', videoId: '-pNOha_rbMw', genre: 'Latin Hits' },
      { title: 'Rakata', artist: 'Wisin & Yandel', videoId: 'EkAJCkR_9cg', genre: 'Latin Hits' },
    ]
  },
  {
    name: 'Indie',
    icon: '🎸',
    songs: [
      { title: 'Heat Waves', artist: 'Glass Animals', videoId: 'mRD0-GxqHVo', genre: 'Indie' },
      { title: 'Gooey', artist: 'Glass Animals', videoId: 'jeo3an2M_Lo', genre: 'Indie' },
      { title: 'Sweater Weather', artist: 'The Neighbourhood', videoId: 'GCdwKhTtNNw', genre: 'Indie' },
      { title: 'Mr. Brightside', artist: 'The Killers', videoId: 'gGdGFtwCNBE', genre: 'Indie' },
      { title: 'Somebody That I Used to Know', artist: 'Gotye', videoId: '8UVNT4wvIGY', genre: 'Indie' },
      { title: 'Take Me Out', artist: 'Franz Ferdinand', videoId: 'Ijk4j-r7qPA', genre: 'Indie' },
      { title: 'Do I Wanna Know?', artist: 'Arctic Monkeys', videoId: 'bpOSxM0rNPM', genre: 'Indie' },
      { title: 'R U Mine?', artist: 'Arctic Monkeys', videoId: 'ngzC_8zqInk', genre: 'Indie' },
      { title: '505', artist: 'Arctic Monkeys', videoId: 'iV5VKdcQOJE', genre: 'Indie' },
      { title: 'Fluorescent Adolescent', artist: 'Arctic Monkeys', videoId: 'ma9I9VBKPiw', genre: 'Indie' },
      { title: 'Electric Feel', artist: 'MGMT', videoId: 'MmZexg8sxyk', genre: 'Indie' },
      { title: 'Kids', artist: 'MGMT', videoId: 'fe4EK4HSPkI', genre: 'Indie' },
      { title: 'Little Dark Age', artist: 'MGMT', videoId: 'rtL5oMyBHPs', genre: 'Indie' },
      { title: 'Tongue Tied', artist: 'Grouplove', videoId: '1x1wjGKHjBI', genre: 'Indie' },
      { title: 'Pumped Up Kicks', artist: 'Foster The People', videoId: 'SDTZ7iX4vTQ', genre: 'Indie' },
      { title: 'Sit Next to Me', artist: 'Foster The People', videoId: 'BKLVpDTZOPQ', genre: 'Indie' },
      { title: 'Stolen Dance', artist: 'Milky Chance', videoId: 'iX-QaNzd-0Y', genre: 'Indie' },
      { title: 'Ophelia', artist: 'The Lumineers', videoId: 'pTOC_q0NLTk', genre: 'Indie' },
      { title: 'Ho Hey', artist: 'The Lumineers', videoId: 'zvCBSSwgtg4', genre: 'Indie' },
      { title: 'The Less I Know The Better', artist: 'Tame Impala', videoId: 'sBzrzS1Ag_g', genre: 'Indie' },
      { title: 'Elephant', artist: 'Tame Impala', videoId: 'LnKUD_OztRE', genre: 'Indie' },
      { title: 'Borderline', artist: 'Tame Impala', videoId: 'p9Yjy1BFZDA', genre: 'Indie' },
      { title: 'Two Door Cinema Club', artist: 'What You Know', videoId: 'YXwYJyrKK5A', genre: 'Indie' },
      { title: 'Undercover Martyn', artist: 'Two Door Cinema Club', videoId: 'LLK4oaXUuLg', genre: 'Indie' },
      { title: 'Young Folks', artist: 'Peter Bjorn and John', videoId: 'OIRE6iw-ws4', genre: 'Indie' },
      { title: 'Home', artist: 'Edward Sharpe', videoId: 'DHEOF_rcND8', genre: 'Indie' },
      { title: 'Skinny Love', artist: 'Bon Iver', videoId: 'ssdgFoHLwnk', genre: 'Indie' },
      { title: 'Holocene', artist: 'Bon Iver', videoId: 'TWcyIpul8OE', genre: 'Indie' },
      { title: 'Ribs', artist: 'Lorde', videoId: '4qaeoz_7cyE', genre: 'Indie' },
      { title: 'Green Light', artist: 'Lorde', videoId: 'dMK_npDG12Q', genre: 'Indie' },
    ]
  },
  {
    name: 'Dance Pop',
    icon: '💃',
    songs: [
      // 2024-2025 Fresh Dance Pop
      { title: 'APT.', artist: 'ROSE & Bruno Mars', videoId: 'ekr2nIex040', genre: 'Dance Pop' },
      { title: 'Die With A Smile', artist: 'Lady Gaga & Bruno Mars', videoId: 'kPa7bsKwL-c', genre: 'Dance Pop' },
      { title: 'Espresso', artist: 'Sabrina Carpenter', videoId: 'eVli-tstM5E', genre: 'Dance Pop' },
      { title: 'Please Please Please', artist: 'Sabrina Carpenter', videoId: 'cF1Na4AIecM', genre: 'Dance Pop' },
      { title: 'Unholy', artist: 'Sam Smith & Kim Petras', videoId: 'Uq9gPaIzbe8', genre: 'Dance Pop' },
      { title: 'Flowers', artist: 'Miley Cyrus', videoId: 'G7KNmW9a75Y', genre: 'Dance Pop' },
      { title: 'About Damn Time', artist: 'Lizzo', videoId: 'IXXxciRUMzE', genre: 'Dance Pop' },
      { title: 'As It Was', artist: 'Harry Styles', videoId: 'H5v3kku4y6Q', genre: 'Dance Pop' },
      { title: 'Vampire', artist: 'Olivia Rodrigo', videoId: 'RlPNh_PBZb4', genre: 'Dance Pop' },
      { title: 'Levitating', artist: 'Dua Lipa ft. DaBaby', videoId: 'WHuBW3qKm9g', genre: 'Dance Pop' },
      { title: 'Houdini', artist: 'Dua Lipa', videoId: 'suAR1PYFNYA', genre: 'Dance Pop' },
      { title: 'Physical', artist: 'Dua Lipa', videoId: '9HDEHj2yzew', genre: 'Dance Pop' },
      { title: 'Rain On Me', artist: 'Lady Gaga & Ariana Grande', videoId: 'AoAm4om0wTs', genre: 'Dance Pop' },
      { title: 'The One', artist: 'Tate McRae', videoId: 'qCwN5qMqy_8', genre: 'Dance Pop' },
      { title: 'Greedy', artist: 'Tate McRae', videoId: '8HnLRrQ3RS4', genre: 'Dance Pop' },
      { title: 'positions', artist: 'Ariana Grande', videoId: 'tcYodQoapMg', genre: 'Dance Pop' },
      { title: '7 rings', artist: 'Ariana Grande', videoId: 'QYh6mYIJG2Y', genre: 'Dance Pop' },
      { title: 'Dynamite', artist: 'BTS', videoId: 'gdZLi9oWNZg', genre: 'Dance Pop' },
      { title: 'Butter', artist: 'BTS', videoId: 'WMweEpGlu_U', genre: 'Dance Pop' },
      { title: 'Pink Venom', artist: 'BLACKPINK', videoId: 'gQlMMD8auMs', genre: 'Dance Pop' },
      { title: 'How You Like That', artist: 'BLACKPINK', videoId: 'ioNng23DkIM', genre: 'Dance Pop' },
      { title: 'Supernova', artist: 'aespa', videoId: 'phuiiNCxRMg', genre: 'Dance Pop' },
      { title: 'Magnetic', artist: 'ILLIT', videoId: 'Vk5-c_v4gMU', genre: 'Dance Pop' },
      { title: 'Whatever', artist: 'Kylie Minogue', videoId: 'Y0iqKDEWcdI', genre: 'Dance Pop' },
      { title: 'Padam Padam', artist: 'Kylie Minogue', videoId: 'TVSx2pyGN4M', genre: 'Dance Pop' },
    ]
  },
];

// Get all songs flattened
export const getAllSongs = (): Song[] => {
  return MUSIC_LIBRARY.flatMap(genre => genre.songs);
};

// Get songs by genre
export const getSongsByGenre = (genreName: string): Song[] => {
  const genre = MUSIC_LIBRARY.find(g => g.name.toLowerCase() === genreName.toLowerCase());
  return genre?.songs || [];
};

// Get a random song
export const getRandomSong = (): Song => {
  const allSongs = getAllSongs();
  return allSongs[Math.floor(Math.random() * allSongs.length)];
};

// Get album art URL from video ID
export const getAlbumArt = (videoId: string): string => {
  return `https://i.ytimg.com/vi/${videoId}/hqdefault.jpg`;
};
